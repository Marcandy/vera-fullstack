...
The rules live in the Java service layer, and the browser asks for answers
rather than working them out. Each half has its own README with the detail:
[the API](java-spring-boot-back-end-app/README.md) and
[the front end](react-front-end-app/README.md).

- React with Vite, React Router and CSS Modules in the browser; Java 21, Spring Boot 4.1, Maven and MySQL behind it. No UI library, no state library, no TypeScript.
- All data access in the browser goes through a service layer (`react-front-end-app/src/services`). Components never fetch and never import data directly, which is what let the API replace the mock internals underneath them. That has now been tested rather than asserted: the visit reads and the caregiver document writes each moved onto HTTP without a single page or component file changing.
- Mutations are domain verbs (`checkInVisit`, `checkOutVisit`, `supplyEvidence`, `submitClaim`, `addCaregiver`, `signDocument`), and the transition rules behind them live in the Java service, not in a controller, not in an entity, and no longer in a component.
